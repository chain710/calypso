...
{
    demo_app_t::http_parser_ctx* ctx = (demo_app_t::http_parser_ctx*)p->data;
    if (ctx->parse_state_ != demo_app_t::http_parser_ctx::parse_field)
    {
        if (!ctx->field_.empty())
        {
            L_DEBUG("field=%s, value=%s", ctx->field_.c_str(), ctx->value_.c_str());
        }
        ...
        ctx->parse_state_ = demo_app_t::http_parser_ctx::parse_field;
        ctx->field_.clear();
    }
...
{
    demo_app_t::http_parser_ctx* ctx = (demo_app_t::http_parser_ctx*) p->data;
    if (ctx->parse_state_ != demo_app_t::http_parser_ctx::parse_value)
    {
        ctx->parse_state_ = demo_app_t::http_parser_ctx::parse_value;
        ctx->value_.clear();
    }
...
{
    demo_app_t::http_parser_ctx* ctx = (demo_app_t::http_parser_ctx*)p->data;
    ctx->parse_state_ = demo_app_t::http_parser_ctx::parse_none;
    return 0;
}
...
void demo_app_t::broadcast_newlog( const std::string& fn, const std::string& logline )
{
...
demo_app_t::demo_app_t()
{
    last_handle_reload_ = 0;
    harvester_.reg_newlog_callback(bind(&demo_app_t::broadcast_newlog, this, placeholders::_1, placeholders::_2));
    harvester_.create();
    last_watch_log_check_time_ = 0;
}

int demo_app_t::load_log_pattern( const char* path )
{
    string json_raw;
...
void demo_app_t::watch_log_check(time_t now)
{
